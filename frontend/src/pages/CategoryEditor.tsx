...
export default function CategoryEditor() {
  ...
  // ── Add ──────────────────────────────────────────────────────────────────────
  const openAdd = () => { setNewName(''); setError(null); setModalOpen(true); };
              ...
              return (
                <tr key={cat.key} style={{ borderBottom: '1px solid #eee', backgroundColor: '#d4edda' }}>
                  ...
                  <td style={{ padding: 12, textAlign: 'center', display: 'flex', gap: 8, justifyContent: 'center' }}>
                    <button onClick={() => openEdit(cat)} style={{ padding: '4px 14px', backgroundColor: '#007bff', color: '#fff', border: 'none', borderRadius: 4, cursor: 'pointer', fontSize: '0.85rem' }}>
                      ערוך
                    </button>
                    <button
